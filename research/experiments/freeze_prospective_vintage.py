...
def freeze_vintage(
    ...
    *,
    ...
    resolved_start = start_date or str(
        json.loads((root / "data/canonical/provenance.json").read_text(encoding="utf-8"))[
            "start_date"
        ]
    )
    universe = load_universe(universe_path or root / "config/universe.csv")
    dataset = download_tushare_dataset(api, universe, resolved_start, as_of_day.strftime("%Y%m%d"))
...
def main() -> None:
    ...
    parser.add_argument("--start-date", help="下载起始日，YYYYMMDD；默认沿用 canonical 起点")
