...
| Equal risk contribution | allocation | E2 | 成熟构造方法，非收益异常 | S4B blocked / S4C | candidate-freeze review eligible | S4C 独立 candidate-freeze review；集中度未消除，S4B block 保留 | UPSTREAM_COMPARE |
...
S1、S3A、S3B、S3C、S4A、S8A 与 S10A 的具体问题已关闭，不能换名或调参重开。S2 R1 是唯一前瞻影子候选；S4C 已完成权威执行审查并获 candidate-freeze review 资格，S27A 获同一资格但继续推迟；两者均非候选、前瞻或生产资格，S4C 的集中度证据仍未消除。S4B 因 Riskfolio-Lib 依赖阻塞的历史结论保持不变。S30 是简单复杂度门槛，不是alpha策略。
